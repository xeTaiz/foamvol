#!/usr/bin/env bash
# Cooperatively claim and run the 270-arm Stage-3 25-view queue.
set -u
...
PY="${PY:-python3}"
CFG_DIR_REL="experiments/multiscene_feature_followup_v1/stage3_25_configs"
QUEUE_REL="experiments/multiscene_feature_followup_v1/stage3_25_queue.txt"
SHARED_RUNNER="$HERE/../sweep_revalidate_v1/run_sweep.sh"
ROOT="$REPO/output/multiscene_feature_followup_v1/stage3_25"
CLAIMS="$ROOT/.claims"
...
while true; do
    ...
    TAG="${ARM#*__}"
    if [ -f "$RUN/DONE" ]; then
        if ! "$PY" "$HERE/assert_stage3_25_active.py" --run "$RUN" --config "$CFG" --tag "$TAG" >>"$RUN/run.log" 2>&1; then
            rm -f "$RUN/DONE"
            touch "$RUN/FAILED"
        fi
    fi
...
printf 'Stage-3 25-view queue drained on %s gpu=%s\n' "$(hostname)" "$GPU"
