#!/usr/bin/env bash
# Run one randomized Stage-1 scene/seed block sequentially on one GPU.
set -eu
...
PY="${PY:-python3}"
CFG_DIR_REL="experiments/multiscene_feature_followup_v1/stage1_configs"
...
printf 'Stage-1 block finished on %s gpu=%s arms=%s\n' "$(hostname)" "$GPU" "${#ARMS[@]}"
