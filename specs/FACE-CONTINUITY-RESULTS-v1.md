...
### Findings
- **Letting geometry keep moving past the split-activation freeze dominates every other lever tested, as measured by the split-aware hard eval (`split_voxelize.py`) — see "Independent verification" below.** Unfreezing points is worth +2.50 dB volume PSNR at 64k and +2.29 dB at 128k (control vs. control) — an order of magnitude larger than any regularizer weight tested, and it also improves every other metric (SSIM3D, Dice, air MAE/FPR, chamfer, HD95, F1@1/F1@2) simultaneously. **A non-split-aware per-cell eval (TensorBoard's `test/vol_raw_psnr`) shows only +0.44 dB on these same checkpoints** — it structurally can't see sub-cell split geometry, so it understates this lever's effect; it does not contradict the hard-eval finding. This is the single largest lever in the whole experiment set, on the eval that can actually see what the split cells do.
...
- Unfreezing points past the split-activation freeze (`points_hard_freeze_at: -1`) is a far larger win (+2.3–2.5 dB volume PSNR, split-aware hard eval) independent of the continuity regularizer; if pursuing further gains, prioritize this over regularizer tuning. Not "unconditional" in the sense of every metric family — the non-split-aware `vol_raw_psnr` eval shows only +0.44 dB on the same checkpoints (see "Independent verification"), because it cannot resolve sub-cell split geometry at all.
